 ...
 * @since 2.0
 */
class Generator2 extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            // service
            $nsName = str_replace('models', 'services', $this->ns);
            var_dump($nsName);
            $serviceClassName = $modelClassName . 'Service';
            $servicePath = Yii::getAlias('@' . str_replace('\\', '/', $nsName)) . '/' . $serviceClassName . '.php';
            if(file_exists($servicePath) == false){ // generated only once
                $params = [
                    'nsName' => $nsName,
                ...
                $files[] = new CodeFile(
                    Yii::getAlias('@' . str_replace('\\', '/', $nsName)) . '/' . $serviceClassName . '.php',
